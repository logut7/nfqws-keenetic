...
> 
> Автор не несёт ответственности за порчу оборудования и программного обеспечения, а так же проблемы с доступом.
...
<details>
  ...
  - TP-Link Archer C6U
  - TP-Link WDR3500
  - Xiaomi Mi Router 3G, 4, 4A, 4C, Mini, Pro
...
- Все дальнейшие команды выполняются не в cli роутера, а **в среде entware**. Подключиться в неё можно несколькими способами:
  ...
  - Или подключиться напрямую через SSH (логин - `root`, пароль по умолчанию - `keenetic`, порт - 222 или 22). Для этого в терминале написать `ssh root@192.168.1.1 -p 222`.
...
2. Установите opkg-репозиторий в систему
   ...
   <details>
     ...
     - `mipsel-3.4` <sub><sup>Keenetic 4G (KN-1212), Giga (KN-1010/1011), Ultra (KN-1810), Viva (KN-1910/1912/1913), Hero 4G (KN-2310/2311), Giant (KN-2610), Skipper 4G (KN-2910), Hopper (KN-3810)</sup></sub>
       ```
       mkdir -p /opt/etc/opkg
       echo "src/gz nfqws-keenetic https://anonym-tsk.github.io/nfqws-keenetic/mipsel" > /opt/etc/opkg/nfqws-keenetic.conf
       ```

     - `mips-3.4` <sub><sup>Keenetic Giga SE (KN-2410), Ultra SE (KN-2510), DSL (KN-2010), Duo (KN-2110), Skipper DSL (KN-2112), Hopper DSL (KN-3610)</sup></sub>
       ```
       mkdir -p /opt/etc/opkg
